...
是的，用 **Git Bash** 安装 Foundry 是 Windows 上官方推荐的第二种方式（第一是 WSL），因为 Foundryup 脚本是 Bash 脚本，不支持 PowerShell 或 CMD，但 Git Bash 完全兼容（它提供 Unix-like 环境，包括 curl 和 bash）。

当前（2026年1月）官方文档（book.getfoundry.sh）明确说：**Windows 用户需要用 Git Bash 或 WSL 作为终端**，Foundryup 不支持 PowerShell/CMD。Git Bash 方式简单、轻量，不用装完整 Linux 子系统，很多开发者（尤其是只想快速上手 Solidity 测试）都用这个。
