...
### ERC-7720
在传统的 EOA（普通钱包）操作中，如果你想在 Uniswap 上把 USDT 换成 PEPE，你需要分两步：先点一次 Approve，再点一次 Swap，不仅麻烦还要付两次 Gas 费。

而在 ERC-7702 的支持下，流程被极致简化了。以下是“一键完成授权并兑换”的具体流程：
#### ERC-7702 “一键多步”流程图
用户签名（授权委派）： 用户签署一个特殊的授权消息，同意在这一笔交易中，将自己的 EOA 地址临时委派给一个“批处理合约”代码。

构造原子交易： 用户只需发送一笔交易。这笔交易同时包含了：

Approve(USDT, Uniswap)：授权给协议。

Swap(USDT, PEPE)：执行兑换逻辑。

临时变身（Execution Context）： 当交易到达以太坊节点时，你的 EOA 地址在执行环境里变成了“智能合约”。它能够理解并顺序执行上述两个指令。

结果返回： 交易完成后，你的 EOA 地址恢复原状。原本需要两次确认的操作，现在一次点击、一次签名、一笔 Gas 费就全部搞定。

#### 为什么这比 ERC-4337 更诱人？
对 DApp 开发者的魔力：开发者可以写一个简单的脚本，让用户“一键清理小额代币余额”或“一键完成流动性挖矿”，用户不需要学习什么是合约钱包，只要继续用他的 MetaMask 即可。

安全性增强：因为这个委派是临时且限定操作的，即使授权代码有 Bug，它也只在这一笔交易里生效，不会像长期授权（Approve Max）那样留下永久的隐患。

这种体验的提升，正是 Vitalik 推动将其纳入 Prague 升级 的主要动力。
...
#### 案例之私钥丢失后的社交恢复
在传统 EOA 钱包（如 MetaMask）中，私钥丢了资产就永远找不回了。但在 ERC-4337 账户抽象钱包中，你可以设定 3 个好友为“守护者”，只要其中 2 人签名，就能帮你重置钱包密钥。

第一步：发起意图 (UserOperation)
好友 A 和 B 并不是直接在链上操作小明的钱包。他们签署一个链下数据包，叫做 UserOperation。

这个包里写着：“我们是守护者，请把小明钱包的新主人地址改为 [新地址]。”

由于这不是标准交易，它不会发往以太坊传统的交易池（Mempool）。

第二步：打包与提交 (Bundler)
UserOp 被发往一个专门的 待处理池（Alt-Mempool）。

Bundler（打包者） 像勤劳的搬运工，从池子里捞出多个 UserOp，把它们塞进一笔真实的以太坊交易中。

Bundler 支付这笔上链交易的 Gas 费（ETH）。

第三步：入口合约校验 (EntryPoint)
以太坊节点将这笔交易发往全球唯一的 EntryPoint 合约。

EntryPoint 首先找到小明的钱包合约，调用它的 validateOp 函数。

校验逻辑：小明的钱包合约会自动检查：“这两个签名是不是来自预设的 A 和 B？如果是，验证通过。”

第四步：执行与结算 (Execution & Refund)
验证通过后，EntryPoint 正式执行指令：修改小明钱包合约中的 owner 变量为新地址。

最后，小明钱包里的余额会自动划扣一部分给 Bundler，作为刚才垫付 Gas 费的报酬。


总结对照步骤传统 EOA (MetaMask)ERC-4337 智能钱包发起直接发送 Signed Transaction发送 UserOperation 意图支付必须自己账户里有 ETH可以由 Bundler 垫付，Paymaster 赞助验证固定为 ECDSA 签名加密可编程逻辑（指纹、多签、社交恢复）上链矿工直接处理Bundler 打包后交给 EntryPoint
