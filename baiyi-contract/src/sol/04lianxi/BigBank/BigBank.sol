...
interface IBank {
    ...
    function getTopDepositors()
        external
        view
        returns (address[3] memory, uint[3] memory);
...
contract Bank is IBank {
    ...
    function updateTopDepositors(address depositor) internal {
        ...
        for (uint8 i = 0; i < TOP_COUNT; i++) {
            ...
            if (
                currentAddr == address(0) ||
                depositorBalance > deposits[currentAddr]
            ) {
    ...
    function _updateRanking() internal {
        for (uint8 i = 1; i < TOP_COUNT; i++) {
            ...
            while (
                j >= 0 &&
                (topDepositors[uint8(j)] == address(0) ||
                    deposits[topDepositors[uint8(j)]] < keyDeposit)
            ) {
